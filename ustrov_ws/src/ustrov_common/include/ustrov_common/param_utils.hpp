...
#define USTROV_COMMON_DECLARE_PARAM_DESCR(x, y) \
  do {                                         \
    auto &param = params_.x;                   \
    param = declare_parameter(#x, param, y);   \
  } while (false)

#define USTROV_COMMON_DECLARE_PARAM(x)     \
  do {                                    \
    auto &param = params_.x;              \
    param = declare_parameter(#x, param); \
  } while (false)

#define USTROV_COMMON_DECLARE_PARAM_DOUBLE_RANGE(x, lower, upper)    \
  do {                                                              \
    auto &param = params_.x;                                        \
...
#define USTROV_COMMON_DECLARE_PARAM_INTEGER_RANGE(x, lower, upper, step_size) \
  do {                                                                       \
    auto &param = params_.x;                                                 \
    rcl_interfaces::msg::ParameterDescriptor descriptor;                     \
...
#define USTROV_COMMON_DECLARE_PARAM_NO_DEFAULT(x)            \
  do {                                                      \
    params_.x = declare_parameter<decltype(params_.x)>(#x); \
  } while (false)

#define USTROV_COMMON_DECLARE_PARAM_READONLY(x)                      \
  do {                                                              \
    auto &param = params_.x;                                        \
    rcl_interfaces::msg::ParameterDescriptor descriptor;            \
...
#define USTROV_COMMON_ASSIGN_SIMPLE_LOG(name, updated, text)                    \
  if (ustrov_common::param_utils::AssignIfMatch(parameter, #name, params_.name, \
                                               text)) {                        \
