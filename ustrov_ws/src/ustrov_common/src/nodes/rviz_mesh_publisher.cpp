 ...
 public:
  MeshPublisher() : Node("mesh_publisher") {
    mesh_url_ = declare_parameter<std::string>(
        "mesh_url", "package://ustrov_sim/models/pool/meshes/pool.dae");
    marker_frame_ =
    ...
    if (declare_parameter<bool>("static_tf", true)) {
      ...
      Eigen::Quaterniond q =
          ustrov_common::tf2_utils::EulerToQuaternion(R, P, Y);
      ...
      ustrov_common::convert::EigenToRos(q, t.transform.rotation);
