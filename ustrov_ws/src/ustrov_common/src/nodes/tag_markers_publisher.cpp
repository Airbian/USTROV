...
class TagMarkersPublisher : public rclcpp::Node {
 public:
  ...
  void add_frame_points(const ustrov_common::yaml::TagPose &pose) {
    ...
    for (const auto &tip : tips) {
      ustrov_common::convert::EigenToRos(pose.position, tmp_point);
      lines_.points.push_back(tmp_point);
      lines_.colors.push_back(rgba.at(i));
      lines_.colors.push_back(rgba.at(i));
      ustrov_common::convert::EigenToRos(tip, tmp_point);
      lines_.points.push_back(tmp_point);
      ++i;
    }
  }
...
int main(int argc, char **argv) {
  ...
  auto node = std::make_shared<ustrov_common::TagMarkersPublisher>();
  rclcpp::spin(node);
  return 0;
}
