...
void TfPublisherVehicle::OnOdometry(
  ...
  tf.child_frame_id = ustrov_common::tf2_utils::frame_id::BaseLink(this);
...
void TfPublisherVehicle::OnVisionPoseCovariance(
  ...
  tf.header.stamp = now();
  tf.header.frame_id = _msg->header.frame_id;
  tf.child_frame_id = ustrov_common::tf2_utils::frame_id::VisionBaseLink(this);
  tf.transform.translation.x = _msg->pose.pose.position.x;
  tf.transform.translation.y = _msg->pose.pose.position.y;
  tf.transform.translation.z = _msg->pose.pose.position.z;
  tf.transform.rotation = _msg->pose.pose.orientation;
...
void TfPublisherVehicle::BroadCastStatic() {
  std::vector<geometry_msgs::msg::TransformStamped> transforms;
  {
    ...
    t.transform = ustrov_common::tf2_utils::FLUtoFRD();
    std::string child_frame =
        ustrov_common::tf2_utils::frame_id::BaseLinkFrd(this);
    t.header.frame_id = ustrov_common::tf2_utils::frame_id::BaseLink(this);
  ...
  // direction) and the frd-baselink
  {
    geometry_msgs::msg::TransformStamped t;
    t.transform = ustrov_common::tf2_utils::FLUtoFRD();
    t.header.frame_id = ustrov_common::tf2_utils::frame_id::VisionBaseLink(this);
    t.child_frame_id =
        ustrov_common::tf2_utils::frame_id::VisionBaseLinkFrd(this);
    transforms.push_back(t);
  }
  ...
  if (params_.has_vertical_camera) {
    ...
    t2.transform = ustrov_common::tf2_utils::CameraLinkToCameraFrame();
    t2.header.frame_id =
        ustrov_common::tf2_utils::frame_id::VerticalCameraLink(this);
    t2.child_frame_id =
        ustrov_common::tf2_utils::frame_id::VerticalCameraFrame(this);
  ...
  if (params_.has_front_camera) {
    ...
    t.header.frame_id = ustrov_common::tf2_utils::frame_id::BaseLink(this);
    t.child_frame_id = ustrov_common::tf2_utils::frame_id::FrontCameraLink(this);
    transforms.push_back(t);
    // same transformation for the vision subtree
    t.header.frame_id = tf2_utils::frame_id::VisionBaseLink(this);
    ...
    t2.transform = ustrov_common::tf2_utils::CameraLinkToCameraFrame();
    t2.header.frame_id =
        ustrov_common::tf2_utils::frame_id::FrontCameraLink(this);
    t2.child_frame_id =
        ustrov_common::tf2_utils::frame_id::FrontCameraFrame(this);
    transforms.push_back(t2);
    // same transformation for the vision subtree
...
RCLCPP_COMPONENTS_REGISTER_NODE(ustrov_common::TfPublisherVehicle)
