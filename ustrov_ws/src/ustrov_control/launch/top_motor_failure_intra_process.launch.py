...
from ustrov_common.launch_helper import (
    LaunchArgsDict,
...
def declare_launch_args(launch_description: LaunchDescription):
    ...
    package_path = get_package_share_path('ustrov_common')
    path = str(package_path / 'config/transformations_ustrov_default.yaml')
    action = DeclareLaunchArgument('tf_config_vehicle_file', default_value=path)
    launch_description.add_action(action)

    package_path = get_package_share_path('ustrov_control')
    path = str(
        package_path
...
def add_composable_nodes(launch_description: LaunchDescription):
    ...
    node = ComposableNode(
        ...
        plugin='ustrov_control::motor_failure::ControlNode',
    ...
    node = ComposableNode(
        package='ustrov_control',
        plugin='ustrov_control::attitude_control::QuaternionControlNode',
        namespace=LaunchConfiguration('vehicle_name'),
    ...
    node = ComposableNode(
        package='ustrov_common',
        plugin='ustrov_common::TfPublisherVehicle',
        namespace=LaunchConfiguration('vehicle_name'),
