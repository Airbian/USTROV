...
#include <ustrov_control/attitude_control/geometric_controller.hpp>
...
void GeometricControlNode::InitPublishers() {
  ...
  using ustrov_control_msgs::msg::ActuatorSetpoint;
...
void GeometricControlNode::InitSubscriptions() {
  ...
  using ustrov_control_msgs::msg::RollTarget;
...
ustrov_control_msgs::msg::ActuatorSetpoint GeometricControlNode::ZeroMsg(
...
void GeometricControlNode::PublishCurrentSetpoint(
  ...
  using ustrov_common::tf2_utils::frame_id::InertialFrame;
...
void GeometricControlNode::OnHeadingTarget(
  ...
  if (_msg->header.frame_id !=
      ustrov_common::tf2_utils::frame_id::InertialFrame()) {
    RCLCPP_WARN_THROTTLE(
        get_logger(), *get_clock(), 1000,
        "Heading target frame is [%s] but only [%s] is handled. Ignoring...",
        _msg->header.frame_id.c_str(),
        ustrov_common::tf2_utils::frame_id::InertialFrame().c_str());
    return;
  }
  ustrov_common::convert::RosToEigen(_msg->vector, heading_target_);
  controller_.SetAngularVelocityTarget(0.0, 0.0, 0.0);
  using ustrov_common::tf2_utils::QuaternionFromHeading;
...
void GeometricControlNode::OnOdometry(
  ...
  using ustrov_common::tf2_utils::frame_id::BaseLink;

  ustrov_control_msgs::msg::ActuatorSetpoint torque_msg;
  torque_msg.header.stamp = t_now;
  torque_msg.header.frame_id = BaseLink(this);
  Eigen::Quaterniond orientation;
  ustrov_common::convert::RosToEigen(_msg->pose.pose.orientation, orientation);
  Eigen::Vector3d body_rates;
  ustrov_common::convert::RosToEigen(_msg->twist.twist.angular, body_rates);
...
RCLCPP_COMPONENTS_REGISTER_NODE(
    ustrov_control::attitude_control::GeometricControlNode)
