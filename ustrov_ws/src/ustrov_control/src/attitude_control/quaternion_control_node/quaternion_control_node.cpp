...
void QuaternionControlNode::InitPublishers() {
  ...
  topic = "~/debug";
  debug_pub_ =
      create_publisher<ustrov_control_msgs::msg::QuaternionControlDebug>(topic,
                                                                        qos);
}
...
void QuaternionControlNode::InitSubscriptions() {
  ...
  // using ustrov_control_msgs::msg::HeadingTarget;
...
void QuaternionControlNode::PublishAngularVelocitySetpoint(
  ...
  msg.header.frame_id = ustrov_common::tf2_utils::frame_id::BaseLink(this);
  ustrov_common::convert::EigenToRos(_velocity, msg.vector);
...
void QuaternionControlNode::PublishDebugMsg(const rclcpp::Time &_now) {
  ...
  Eigen::Quaterniond q_error =
      ustrov_common::tf2_utils::RotationBetweenNormalizedVectors(
          current_heading, target_heading_);
  msg.angle_error = ustrov_common::tf2_utils::AngleOfRotation(q_error);
  debug_pub_->publish(msg);
}
// void QuaternionControlNode::OnHeadingTarget(
//     const ustrov_control_msgs::msg::HeadingTarget::SharedPtr _msg) {
//   ustrov_common::convert::RosToEigen(_msg->heading, target_heading_);
//   ustrov_common::convert::RosToEigen(_msg->current_orientation, orientation_);
...
void QuaternionControlNode::OnHeadingTarget(
  ...
  ustrov_common::convert::RosToEigen(_msg->vector, target_heading_);
}

void QuaternionControlNode::OnOdometry(
    const nav_msgs::msg::Odometry::SharedPtr _msg) {
  ustrov_common::convert::RosToEigen(_msg->pose.pose.orientation, orientation_);
  const Eigen::Vector3d desired_body_rates =
      attitude_controller_.Update(target_heading_, 0.0, orientation_);
  ...
  Eigen::Quaterniond q =
      ustrov_common::tf2_utils::QuaternionFromHeading(target_heading_, 0.0);
  geometry_msgs::msg::Pose pose;
  pose.position = _msg->pose.pose.position;
  ustrov_common::convert::EigenToRos(q, pose.orientation);
...
RCLCPP_COMPONENTS_REGISTER_NODE(
    ustrov_control::attitude_control::QuaternionControlNode)
