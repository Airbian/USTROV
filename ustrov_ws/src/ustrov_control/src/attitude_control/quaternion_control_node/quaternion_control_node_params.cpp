...
void QuaternionControlNode::DeclareParams() {
  USTROV_COMMON_DECLARE_PARAM_NO_DEFAULT(gain);
  USTROV_COMMON_DECLARE_PARAM_NO_DEFAULT(roll_weight);
...
rcl_interfaces::msg::SetParametersResult QuaternionControlNode::OnParameters(
  ...
  for (const auto &parameter : _parameters) {
    USTROV_COMMON_ASSIGN_SIMPLE_LOG(gain, updated, text);
    USTROV_COMMON_ASSIGN_SIMPLE_LOG(roll_weight, updated, text);
  }
  if (updated) {
