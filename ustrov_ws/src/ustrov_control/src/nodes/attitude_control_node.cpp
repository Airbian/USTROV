...
#include <ustrov_control_msgs/msg/attitude_target.hpp>
...
using namespace ustrov_control::attitude_control;
...
class AttitudeControlNode : public rclcpp::Node {
 public:
  ...
  ustrov_control_msgs::msg::ActuatorSetpoint ZeroMsg(rclcpp::Time _stamp) {
  ...
  void OnAttitudeTarget(const AttitudeTarget::SharedPtr _msg) {
    ...
    ustrov_common::convert::RosToEigen(attitude_target_.attitude, attitude);
    controller_.SetOrientationTarget(attitude);
    setpoint_pub_->publish(attitude_target_);
    if (feedthrough_) {
      ustrov_control_msgs::msg::ActuatorSetpoint thrust_msg;
