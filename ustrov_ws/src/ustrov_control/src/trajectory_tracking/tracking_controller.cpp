...
#include "ustrov_control/trajectory_tracking/tracking_controller.hpp"
...
void TrackingControllerNode::InitPublishers() {
  ...
  attitude_target_pub_ =
      create_publisher<ustrov_control_msgs::msg::AttitudeTarget>(topic, 10);
}
...
void TrackingControllerNode::OnOdometry(
  ...
  ustrov_common::convert::RosToEigen(_msg->pose.pose.position, position_);
  ustrov_common::convert::RosToEigen(_msg->twist.twist.linear, velocity_);
  ustrov_common::convert::RosToEigen(_msg->pose.pose.orientation, attitude_);
...
void TrackingControllerNode::PublishAttitudeTarget(
  ...
  auto msg = std::make_unique<ustrov_control_msgs::msg::AttitudeTarget>();
  msg->header.stamp = _now;
  msg->header.frame_id = ustrov_common::tf2_utils::frame_id::InertialFrame();
  ustrov_common::convert::EigenToRos(_attitude, msg->attitude);
  msg->thrust = _thrust;
...
RCLCPP_COMPONENTS_REGISTER_NODE(
    ustrov_control::trajectory_tracking::TrackingControllerNode)
