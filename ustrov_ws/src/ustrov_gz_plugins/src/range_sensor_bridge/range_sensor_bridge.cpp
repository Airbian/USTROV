...
RangeSensor::RangeSensor(rclcpp::NodeOptions const &_options)
  ...
  //      DeclareParams();
  ranges_pub_ = create_publisher<ustrov_control_msgs::msg::RangeMeasurementArray>(
...
void RangeSensor::OnRanges(
    const gz::ustrov_gz_plugins::RangeMeasurementArray &_msg) {
  rclcpp::Time t_now = now();
  ustrov_control_msgs::msg::RangeMeasurementArray ros_msg;
  ros_msg.header.stamp = t_now;
  for (int i = 0; i < _msg.measurements_size(); ++i) {
    ustrov_control_msgs::msg::RangeMeasurement ros_meas;
...
RCLCPP_COMPONENTS_REGISTER_NODE(
    ustrov_gz_plugins::range_sensor_bridge::RangeSensor)
