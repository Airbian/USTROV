 ...
 private:
  ...
  void DeclareParameters() {
    ...
    descr =
        ...
        "camera data).";
    param = ustrov_common::param_utils::DescriptionLimit(descr, 0, 1000);
    ...
    param = ustrov_common::param_utils::DescriptionLimit(descr, 0.0, 0.5);
    ...
    name = "std_dev.orientation";
    descr = "Standard deviation of the simulated orientation measurement.";
    param = ustrov_common::param_utils::DescriptionLimit(descr, 0.0, 0.5);
    params_.std_dev.orientation =
        declare_parameter<double>(name, 0.02, param);
  ...
  void OnOdometry(const nav_msgs::msg::Odometry::SharedPtr _msg) {
    ...
    pose_msg.pose.covariance[0] = std::pow(params_.std_dev.x, 2);
    pose_msg.pose.covariance[7] = std::pow(params_.std_dev.y, 2);
    pose_msg.pose.covariance[14] = std::pow(params_.std_dev.z, 2);
    pose_msg.pose.covariance[21] =
        std::pow(params_.std_dev.orientation, 2);
  ...
  rcl_interfaces::msg::SetParametersResult OnParams(
    ...
    for (const auto &parameter : _parameters) {
      if (ustrov_common::param_utils::AssignIfMatch(parameter, "delay_ms",
                                                   params_.delay_ms)) {
        result.reason = "Set delay_ms.";
        continue;
      }
      if (ustrov_common::param_utils::AssignIfMatch(parameter, "std_dev.x",
                                                   params_.std_dev.x)) {
        result.reason = "Set std_dev.x";
        continue;
      }
      if (ustrov_common::param_utils::AssignIfMatch(parameter, "std_dev.y",
                                                   params_.std_dev.y)) {
        result.reason = "Set std_dev.y";
        continue;
      }
      if (ustrov_common::param_utils::AssignIfMatch(parameter, "std_dev.z",
                                                   params_.std_dev.z)) {
        result.reason = "Set std_dev.z";
        continue;
      }
      if (ustrov_common::param_utils::AssignIfMatch(
              parameter, "std_dev.orientation",
              params_.std_dev.orientation)) {
        result.reason = "Set std_dev.orientation";
        continue;
      }
    }
    return result;
