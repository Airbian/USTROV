...
#include <stdint.h>
#include <limits>
#define EIGEN_MATRIXBASE_PLUGIN <ustrov_state_estimation/matrix_base_addon.h>
...
struct StateSample {
  ...
  Eigen::Vector3d delta_angle_bias{Eigen::Vector3d::Zero()};
  Eigen::Vector3d delta_velocity_bias{Eigen::Vector3d::Zero()};
};
...
struct ImuSample {
  ...
  double delta_angle_dt{0.0};
  double delta_velocity_dt{0.0};
...
struct VisionSample {
  ...
  Eigen::Vector3d velocity{Eigen::Vector3d::Zero()};
  Eigen::Quaterniond orientation{Eigen::Quaterniond::Identity()};
  Eigen::Vector3d position_variance{Eigen::Vector3d::Constant(0.01)};
  Eigen::Vector3d velocity_variance{Eigen::Vector3d::Constant(0.01)};
...
struct Settings {
  ...
  //////////////////////////////////////////////////////////////////////////////
  // state limits
  Eigen::Vector3d position_upper_limit = Eigen::Vector3d::Constant(
      std::numeric_limits<double>::infinity());
  Eigen::Vector3d position_lower_limit = Eigen::Vector3d::Constant(
      -std::numeric_limits<double>::infinity());
