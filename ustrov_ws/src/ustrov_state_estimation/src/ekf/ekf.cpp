...
bool Ekf::InitFilter() {
  ...
  if (baro_buffer_.PopFirstOlderThan(imu_sample_delayed_.time_us,
    ...
    if (baro_sample_delayed_.time_us != 0) {
      ...
      // 启动初期延迟缓冲区尚未填满是正常状态，不应刷出告警。
      EKF_DEBUG("Delayed baro sample has invalid timestamp!");
    }
  }

  if (baro_counter_ < observation_buffer_length_) {
    EKF_DEBUG("Waiting for more barometric data. Having (%d/%d).",
              baro_counter_, observation_buffer_length_);
    return false;
  } else {
...
void Ekf::PredictState() {
  ...
  const Eigen::Quaterniond delta_quat =
      QuaternionFromDeltaAngle(delta_angle_corrected);
...
void Ekf::CalculateOutputState(const ImuSample &imu_sample) {
  ...
  const Eigen::Quaterniond delta_quat = QuaternionFromDeltaAngle(delta_angle);
  ...
  if (imu_updated_) {
    ...
    // 延迟状态与实时输出之间使用一阶互补修正。这里不能累积误差，
    // 否则恒定的位置观测会产生积分饱和，使输出持续越过真实位置。
    const Eigen::Vector3d velocity_correction = velocity_error * velocity_gain;
    const Eigen::Vector3d position_correction = position_error * position_gain;
...
void Ekf::FuseOrientation() {
  ...
  // 四元数在状态中按四个分量建模，所以这里直接使用分量残差。
  // 对残差再次归一化会把任意微小误差放大成单位长度。
  Eigen::Vector4d innovation = {
      state_.orientation.w() - orientation.w(),
...
bool Ekf::FuseHorizontalPosition(const Eigen::Vector3d &innovation,
  ...
  if (innovation_check_pass || inhibit_gate) {
    ...
    EKF_DEBUG("Refusing horizontal position.");
...
void Ekf::UpdateVisionFusion() {
  if (vision_data_ready_) {
    EKF_DEBUG("Vision data ready to fuse.");
    ...
    if (control_status_.flags.vision_position) {
      EKF_DEBUG("Fusing vision position.");
      Eigen::Vector3d vision_observation_var;
      Eigen::Vector2d vision_innovation_gate;
